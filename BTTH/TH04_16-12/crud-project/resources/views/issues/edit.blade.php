...
<h1 style="margin: 50px 50px">Update Information of Issue</h1>

<form action="{{ route('issues.update', issues->id) }}" method="POST" style="margin: 50px 50px">
    @csrf
    @method('PUT')
    <div class="form-group">
        <label for="title">Computer Name</label>
        <input type="text" name="title" class="form-control" value="{{ issues->title }}" required>
    </div>
    <div class="form-group">
